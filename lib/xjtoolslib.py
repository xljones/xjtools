...
def _rename_tool(tool_name, new_tool_name):
    ...
    if not os.path.exists(filepath):
        ...
        raise FileExistsError("There is already a tool called '{0}', choose another new name".format(new_tool_name))

    # if no errors raised, rename the tool
    else:
        confirmation = input("{0} Are you sure you want to rename '{1}' to '{2}'? [Y/n]: ".format(xjconst._PRINT_PREFIX, tool_name, new_tool_name))
